...
.result-positive { background: #f0fdf4; border: 1.5px solid #10b981; }
.result-negative { background: #fef2f2; border: 1.5px solid #ef4444; }
.result-neutral  { background: #fffbeb; border: 1.5px solid #f59e0b; }
.result-label {
    font-family: 'Space Mono', monospace;
    font-size: 1.3rem;
    font-weight: 700;
...
with st.sidebar:
    ...
    Capstone project comparing 7 NLP and ML models for customer feedback classification.
    <br><br>
    <b>Dataset:</b> Women's E-Commerce Clothing Reviews (22,641 reviews)
    <br><br>
    <b>Tasks:</b> Sentiment classification and complaint category detection.
    </div>
    """, unsafe_allow_html=True)

# ── Header ────────────────────────────────────────────────────────────────────
st.markdown('<p class="main-title">Customer Feedback Analyser</p>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">Comparative NLP and ML Analysis — Capstone Project</p>', unsafe_allow_html=True)
...
if page == "Classifier":
    ...
    for i, ex in enumerate(examples):
        if cols[i % 3].button(ex[:42] + "...", key=f"ex_{i}"):
            ...
            try:
                ...
                <div class="result-box {css_class}">
                    ...
                    <p style="font-size:0.78rem; color:#6b7280; margin-top:0.75rem;">
                        Sentiment model: Linear SVM &nbsp;|&nbsp; Category model: LightGBM
                    </p>
                </div>
                """, unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with c3:
        ...
        r_labels = [f'{k} star' for k in STATS['rating_counts'].keys()]
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown("Six classical ML models plus DistilBERT were trained and evaluated on two tasks.")
    ...
    with tab1:
        ...
        bar_c    = ['#f59e0b' if v == max(f1s_s) else '#3b82f6' if m == 'DistilBERT' else '#6d28d9'
                    for m, v in zip(models_s, f1s_s)]
        ax.bar(models_s, f1s_s, color=bar_c, edgecolor='white')
        ax.set_ylim(0.55, 1.0)
        ax.set_ylabel('F1 Score (weighted)')
        ax.set_title('Sentiment — F1 Score by Model', fontweight='bold')
        ...
        patches = [
            ...
            mpatches.Patch(color='#6d28d9', label='Classical model'),
        ]
    ...
    with tab2:
        ...
        bar_c2   = ['#f59e0b' if v == max(f1s_c) else '#6d28d9' for v in f1s_c]
        ax2.bar(models_c, f1s_c, color=bar_c2, edgecolor='white')
        ax2.set_ylim(0.55, 1.05)
        ax2.set_ylabel('F1 Score (weighted)')
        ax2.set_title('Category — F1 Score by Model', fontweight='bold')
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    <div class="insight-card">
        ...
        <b>Sizing and Fit accounts for 79.9% of all tagged reviews.</b> This is the single highest-priority area.
        Invest in detailed size guides, measurement charts, and user-generated fit feedback such as reviewer height and size bought.
    </div>
    <div class="insight-card amber">
        <div class="insight-title">At-Risk Department</div>
        <b>The Trend department has the lowest average rating at 3.84</b> — well below the 4.18 overall average.
        This department needs a product quality audit. It may be a sourcing problem or a mismatch between product descriptions and what customers actually receive.
    </div>
    <div class="insight-card green">
        <div class="insight-title">Strong Performer</div>
        <b>Bottoms and Intimates lead with ratings of 4.27 to 4.28.</b> Study what works there — fit consistency and fabric quality —
    ...
    <div class="insight-card">
        ...
        <b>LightGBM for category classification (F1 = 0.970) and DistilBERT for sentiment (F1 = 0.839).</b>
        Both are lightweight at inference time and can be integrated into any CRM or order management system without GPU requirements.
    </div>
    <div class="insight-card amber">
        ...
        could be built directly on this signal. Target the 18.1% who would not recommend with follow-up surveys to identify root causes.
    </div>
    """, unsafe_allow_html=True)
